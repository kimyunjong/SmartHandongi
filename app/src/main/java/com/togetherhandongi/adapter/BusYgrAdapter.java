	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (getItem(position).getDivider()==false) {
			...
			if (getItem(position).getMerge_code() == 1) {
				...
				holder.s4.setTextColor(Color.parseColor("#555555"));
			}
				...
				holder.s2.setTextColor(Color.parseColor("#555555"));
		...
		if (!getItem(position).getDivider()) {
			if (toInt(format) > toInt(getItem(position).getS1())) {
				holder.s1.setTextColor(Color.parseColor("#cdcdcd"));
			}
			else {
				holder.s1.setTextColor(Color.parseColor("#555555"));	
				past_now = true;
			}
			holder.s1.setText(convert(getItem(position).getS1()));
			
			if (toInt(format) > toInt(getItem(position).getS2())) {
				holder.s2.setTextColor(Color.parseColor("#cdcdcd"));
			}
			else {
				holder.s2.setTextColor(Color.parseColor("#555555"));
				past_now = true;
			...
			if (toInt(format) > toInt(getItem(position).getS3())) {
				holder.s3.setTextColor(Color.parseColor("#cdcdcd"));
			}
			else {
				holder.s3.setTextColor(Color.parseColor("#555555"));
				past_now = true;
			}
			holder.s3.setText(convert(getItem(position).getS3()));
			
			if (toInt(format) > toInt(getItem(position).getS4())) {
				holder.s4.setTextColor(Color.parseColor("#cdcdcd"));
			}
			else {
				holder.s4.setTextColor(Color.parseColor("#555555"));
				past_now = true;
			...
			if (toInt(format) > toInt(getItem(position).getS5())) {
				holder.s5.setTextColor(Color.parseColor("#cdcdcd"));
			}
			else {
				holder.s5.setTextColor(Color.parseColor("#555555"));
				past_now = true;
			}
